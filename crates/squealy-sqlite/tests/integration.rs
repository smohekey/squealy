...
use squealy::*;
use squealy_sqlite::{SqliteConnection, SqliteError};
...
async fn fixture(sql: &str) -> SqliteConnection {
	let conn = tokio_rusqlite::Connection::open_in_memory()
		.await
		.expect("open fixture database");
	let sql = format!("PRAGMA foreign_keys = ON; {sql}");
	conn
		.call(move |conn| conn.execute_batch(&sql))
		.await
		.expect("create fixture tables");
	SqliteConnection::new(conn)
}
...
async fn foreign_keys_are_enforced() {
	...
	// Inserting a book that references a non-existent author must fail with foreign keys enabled.
