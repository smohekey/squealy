...
/// The user-written definition of a view: its declared output columns plus the `SELECT` that produces
/// them. `#[derive(View)]` generates everything except [`definition`](Self::definition), which the user
/// writes against [`ModelConn`] using [`project`](crate::SourceQuery::project).
///
/// The return type `impl ViewSelect<Row = Self::Row>` is the compile-time guarantee: the body's
/// projection must decode to the same row type as the declared columns, so a mismatch is a type error.
