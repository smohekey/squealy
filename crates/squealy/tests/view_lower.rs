...
fn lowers_filtered_projection_with_inlined_literals() {
	...
	assert!(matches!(&model.projection[1].expr, ExprNode::Column { column, .. } if column == "name"));
...
fn lowers_window_function() {
	...
	let query = conn
		.from::<User>()
		.project(|(user,)| row_number().over(|w| w.partition_by(user.name).order_by(user.id.asc())));
	...
	match &model.projection[0].expr {
		ExprNode::Window {
			...
			assert!(matches!(&partition_by[0], ExprNode::Column { column, .. } if column == "name"));
