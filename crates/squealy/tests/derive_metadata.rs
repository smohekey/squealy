...
fn source_chain_lowers_into_typed_sink_events() {
	...
	sink
		.push_projection::<(i32, String), _>(projection)
		.unwrap();
...
fn insert_query_builds_column_bindings() {
	let columns = HNil.push_back(
		InsertAssignment::<UserName, StaticAssignmentValue<String>>::new(StaticAssignmentValue::new(
			"Ada".to_owned(),
		)),
	);
...
fn update_query_builds_column_bindings_and_filters() {
	...
	let columns = HNil.push_back(
		UpdateAssignment::<UserName, StaticAssignmentValue<String>>::new(StaticAssignmentValue::new(
			"Ada".to_owned(),
		)),
	);
	let filters = HNil.push_back(user.id.equals(1));
	let update = <<TestConnection as QueryBuilder>::Update<'_, User, (), _, _, ()> as UpdateQuery<
		'_,
		_,
		_,
		(),
...
fn prepared_param_values_encode_through_param_writer() {
	...
	let mut all = Vec::new();
	{
		...
		<(String, i32) as PreparedParamValues<Params, TestBackend>>::write_params(&params, &mut writer)
			.unwrap();
	}
	assert_eq!(
		all,
	...
	let mut borrowed_params = Vec::new();
	{
		...
		<(&str, i32) as PreparedParamValues<Params, TestBackend>>::write_params(&borrowed, &mut writer)
			.unwrap();
	}
	assert_eq!(
		borrowed_params,
