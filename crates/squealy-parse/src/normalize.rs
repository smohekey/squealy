...
//! [`squealy_ir::ExprNode`], and both live here (the lowering in [`crate::lower`] calls into them):
//!
//! 1. **Dialect-spelling folds** — the same neutral operation is written differently per dialect. The
//!    renderer chooses the spelling via a `Dialect` hook; normalization inverts that choice
...
//!   (MySQL) → [`squealy_ir::ExprNode::ScalarFn`] `Concat`. Semantics match (both propagate `NULL` in the dialects
...
//!   `SUBSTRING(s FROM start FOR len)` → [`squealy_ir::ExprNode::ScalarFn`] `Substring`.  (~592)
//! - **`now()`**: `CURRENT_TIMESTAMP` vs `CURRENT_TIMESTAMP(6)` (MySQL, `now_fractional_digits`) →
//!   [`squealy_ir::ExprNode::Now`].  (~624)
//! - **Character length**: `length` (SQLite, `unary_string_fn_name`) vs `CHAR_LENGTH` →
//!   [`squealy_ir::ExprNode::ScalarFn`] `Length`.  (helper `scalar_func_name`)
...
//! - **`CAST(<call> AS ty)` result-pins**: [`squealy_ir::ExprNode::Aggregate`]/[`squealy_ir::ExprNode::Window`]/
//!   [`squealy_ir::ExprNode::Extract`]/[`squealy_ir::ExprNode::ExtractSecond`] are wrapped in an outer `CAST` when their
...
//!   `integer_division_needs_float_cast`) → a plain [`squealy_ir::ExprNode::Binary`] `Divide`.  (~274)
//! - **Empty `IN ()`**: rendered as `(<op> IS NOT NULL AND 1 = 0)` (or `… OR 1 = 1` when negated),
//!   since SQL has no `IN ()`. Recognize the sentinel and recover an empty [`squealy_ir::ExprNode::In`].  (~373)
//! - **`FLOOR(EXTRACT(SECOND …))`**: the `Second` field of [`squealy_ir::ExprNode::Extract`] is wrapped in `FLOOR`
//!   (whole-second extraction, distinct from fractional [`squealy_ir::ExprNode::ExtractSecond`]).  (~646)
