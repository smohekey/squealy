...
/// MySQL's `DECIMAL(65, 0)` to the 128-bit-integer representative). `Ok(None)` for any other spelling — the
/// pin inverse is correct there (bare `numeric` → the 128-bit representative, `SIGNED` → `I64`, …). A
/// precision/scale beyond the model's `u32` range is `Err(NotYetLowered)` (kept `Raw`) rather than silently
/// truncated by a wrapping cast.
fn general_cast_decimal_type(data_type: &DataType) -> Result<Option<SqlType>, ReadError> {
    ...
    let (p, s) = match data_type {
        ...
        | DataType::Dec(PrecisionAndScale(p, s)) => (*p, *s),
        _ => return Ok(None),
    };
    match (u32::try_from(p), u32::try_from(s)) {
        (Ok(precision), Ok(scale)) => Ok(Some(SqlType::Decimal { precision, scale })),
        _ => Err(not_yet(format!(
            "general cast to `{data_type}` has a decimal precision/scale beyond the model's range, kept raw"
        ))),
    }
}
...
fn invert_pg_cast_type(data_type: &DataType) -> Option<SqlType> {
    ...
    let ty = match data_type {
        ...
        | DataType::Dec(PrecisionAndScale(p, s)) => SqlType::Decimal {
            // An out-of-`u32`-range precision/scale is unrepresentable — keep the cast `Raw`, not truncated.
            precision: u32::try_from(*p).ok()?,
...
fn invert_mysql_cast_type(data_type: &DataType) -> Option<SqlType> {
    ...
    let ty = match data_type {
        ...
        // exact precision is the backend PR's `canonical_view_*` job). An out-of-`u32`-range precision/scale
        // yields `None` (kept `Raw`) rather than a wrapped value.
        DataType::Decimal(info) => return decimal_from_exact(info),
        _ => return None,
    };
    Some(ty)
}
...
/// `None` when the parsed precision/scale exceeds the model's `u32` range (kept `Raw` rather than truncated
/// by a wrapping cast).
fn decimal_from_exact(info: &sqlparser::ast::ExactNumberInfo) -> Option<SqlType> {
    use sqlparser::ast::ExactNumberInfo;
    let (precision, scale) = match info {
        ExactNumberInfo::None => (10, 0),
        ExactNumberInfo::Precision(p) => (u32::try_from(*p).ok()?, 0),
        ExactNumberInfo::PrecisionAndScale(p, s) => {
            (u32::try_from(*p).ok()?, u32::try_from(*s).ok()?)
        }
    };
...
fn invert_sqlite_cast_type(data_type: &DataType) -> Option<SqlType> {
    match data_type {
        ...
        // `Decimal` re-renders to the same `NUMERIC`. An out-of-`u32`-range precision/scale yields `None`.
        DataType::Numeric(info) => decimal_from_exact(info),
        _ => None,
    }
}
...
fn lower_select(
    ...
    for (index, item) in select.projection.iter().enumerate() {
        ...
        let (output_name, internal_alias) = match (output_names, column_list_is_fallback) {
            ...
            // projection's own explicit `AS`. Keep that alias: the body's own `ORDER BY`/`GROUP BY`/`HAVING`
            // reference the inner alias, so the renderer must re-emit `AS <alias>` (else the reference
            // dangles and the DDL is invalid). It is kept even when it *coincides* with the declared column
            // name — the column list does not introduce that name into the `SELECT` scope, so a bare
            // `SELECT 1 (total) ORDER BY total` still needs the explicit `AS total`. Only an *explicit* alias
            // is kept: a bare column's own name is not a suppressible `AS`, and treating it as one would
            // spuriously re-alias `SELECT q.amount … (n)` as `q.amount AS amount`.
            (Some(names), false) => (names[index].clone(), explicit_alias),
...
/// Resolves every `WHEN`/`THEN` arm and the optional `ELSE` of a `CASE` body against `alias`, protecting
/// the enclosing scope's computed-projection aliases (see [`resolve_expr_columns_protected`]).
...
mod tests {
    ...
    fn general_cast_covers_scalar_functions_but_skips_lossy_decimal() {
        ...
        // A precision/scale beyond the model's `u32` range is kept `Raw` rather than silently truncated by a
        // wrapping cast (`4294967306` would wrap to `10`). Covered for the general PrecisionAndScale path and
        // the pin fallback (a precision-only decimal).
        assert!(matches!(
            low("CAST(`x` AS DECIMAL(4294967306, 2))", SqlDialect::Mysql),
            Err(ReadError::NotYetLowered(_))
        ));
        assert!(matches!(
            low("CAST(`x` AS DECIMAL(4294967306))", SqlDialect::Mysql),
            Err(ReadError::NotYetLowered(_))
        ));
    }
    ...
    #[test]
    fn column_list_alias_referenced_nested_in_having_stays_bare() {
        // A valid SQLite view whose `HAVING` references a computed projection alias *nested* in a
        // comparison. The column list renames the output (`n`), but the inner alias (`total`) must survive
        // and the `HAVING` term must stay bare (not be bound to a nonexistent source column) so the view
        // re-renders valid (git-bug e1d0724, nested-reference case).
        let query = low_query(
            "CREATE VIEW \"v\" (\"n\") AS SELECT SUM(\"q0_0\".\"amount\") AS \"total\" \
             FROM \"events\" AS \"q0_0\" GROUP BY \"q0_0\".\"id\" HAVING \"total\" > 0",
            SqlDialect::Sqlite,
        )
        .unwrap();
        assert_eq!(query.projection[0].output_name, "n");
        assert_eq!(query.projection[0].internal_alias.as_deref(), Some("total"));
        // The `HAVING total > 0` term keeps `total` bare — it names the projection alias, not a source
        // column that would resolve to `q0_0.total`.
        assert_eq!(
            query.having,
            Some(ExprNode::Compare {
                op: CompareOp::GreaterThan,
