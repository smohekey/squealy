...
pub trait Dialect {
	...
	///
	/// This is the spelling for a **result-pin** cast, which fixes a view/query output's wire type, so a
	/// many-to-one representative (bare `DECIMAL`, `SIGNED`) is correct. A **general** authored cast renders through
	...
	/// authored semantics). The default rejects only the universally un-renderable
...
/// PostgreSQL renders bare `numeric`, MySQL `DECIMAL(65, 0)`, and SQLite `INTEGER`, so none preserves
/// the requested integer type exactly. Rendering rejects it rather than emit a lossy cast; author it as a
/// [`ExprNode::Raw`](crate::ExprNode::Raw) if a
