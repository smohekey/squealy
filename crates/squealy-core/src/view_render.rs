//! Dialect-driven rendering of structural view and CTE query bodies.

use std::io::{self, Write};

use crate::{
	AggregateFunc, ArithmeticOp, CteModel, DateField, Dialect, ExprNode, JoinKind, LogicalOp,
...
fn render_set(
	...
	for order in order_by {
		let resolves = match &order.expr {
			ExprNode::BareColumn { column } => outputs.iter().any(|item| &item.output_name == column),
		...
		match body {
			...
			ViewBody::With { ctes, body, .. } => {
				...
				if ctes.iter().any(|inner| inner.name == name) {
					...
					ctes.iter().any(|inner| body_references(&inner.body, name)) || body_references(body, name)
				}
			}
		}
	}
