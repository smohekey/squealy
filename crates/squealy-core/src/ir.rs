...
fn map_scope_exprs_mut(expr: &mut ExprNode, f: &mut impl FnMut(&mut ExprNode)) {
	...
	match expr {
		...
		ExprNode::Not(operand) | ExprNode::IsNull { operand, .. } => map_scope_exprs_mut(operand, f),
		ExprNode::Like {
			operand, pattern, ..
		} => {
			map_scope_exprs_mut(operand, f);
		...
		ViewBody::With {
			...
			for (index, cte) in cte_defs.iter_mut().enumerate() {
				...
				for (other, (name, columns)) in local.iter().enumerate() {
					let visible =
						other < index || (forward && other > index) || (other == index && recursive);
				...
				canonicalize_view_body_scoped(&mut cte.body, ctx, &body_scope, !cte.columns.is_empty());
			}
...
fn map_expr_result_pins(expr: &mut ExprNode, f: &impl Fn(&SqlType) -> SqlType) {
	match expr {
		...
		ExprNode::Not(operand) | ExprNode::IsNull { operand, .. } => map_expr_result_pins(operand, f),
		ExprNode::Like {
			operand, pattern, ..
		} => {
			map_expr_result_pins(operand, f);
	...
	match body {
		...
		ViewBody::With { ctes, body, .. } => {
			...
			sources.extend(
				inner
					.into_iter()
					.filter(|source| source.schema.is_some() || !bound.contains(&source.name.as_str())),
			);
		}
	}
}
...
mod tests {
	...
	fn on_update_shape_error_accepts_now_on_a_temporal_column() {
		assert!(on_update_column(timestamp(), None)
			.on_update_shape_error()
			.is_none());
		assert!(on_update_column(timestamp(), Some(ExprNode::Now))
			.on_update_shape_error()
			.is_none());
	}
	...
	fn on_update_shape_error_rejects_a_non_temporal_column() {
		assert!(on_update_column(SqlType::I32, Some(ExprNode::Now))
			.on_update_shape_error()
			.is_some());
	}
