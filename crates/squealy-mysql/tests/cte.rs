//! MySQL rendering of non-recursive CTEs: referencing a `#[derive(CTE)]` type emits its body as an
//! automatic `WITH `name` (`cols`) AS (…)` prefix, with MySQL backtick-quoted identifiers.
...
#[test]
fn mysql_nested_cte_pulls_dependency_in_first() {
	let query = Mysql.from::<TopUser>().select(|(top,)| (top.id, top.name));

	assert_eq!(
		query.to_sql(),
		"WITH `active_users` (`id`, `name`) AS (\
SELECT q0_0.`id`, q0_0.`name` FROM `shop`.`users` AS q0_0 WHERE (q0_0.`active` = TRUE)), \
...
#[test]
fn mysql_multiple_ctes_across_from_and_join() {
	let query = Mysql
		.from::<ActiveUser>()
		.join::<BigOrder>()
...
SELECT q0_0.`user_id`, q0_0.`total` FROM `shop`.`orders` AS q0_0 WHERE (q0_0.`total` > 100)) \
