...
use squealy::{
	...
	RenderPredicateNodes, RenderProjectable, RenderSelectAst, RenderUpdateAssignments, RowsAffected,
...
{
	fn encode(&self, out: &mut MysqlParamWriter<'_>) -> Result<(), MysqlError> {
		let json = serde_json::to_vec(&self.0).map_err(|_| MysqlError::Conversion("serialize json"))?;
	...
	T: serde::de::DeserializeOwned,
{
	fn decode(row: &mut <Mysql as Backend>::RowReader<'_>) -> Result<Self, MysqlError> {
		let bytes = row.take::<Vec<u8>>()?;
		let inner =
			serde_json::from_slice(&bytes).map_err(|_| MysqlError::Conversion("deserialize json"))?;
		Ok(Json(inner))
	}
}
...
impl Encode<Mysql> for chrono::DateTime<chrono::Utc> {
	fn encode(&self, out: &mut MysqlParamWriter<'_>) -> Result<(), MysqlError> {
		...
		let year =
			u16::try_from(self.year()).map_err(|_| MysqlError::Conversion("chrono year out of range"))?;
		out.push(Value::Date(
			year,
			self.month() as u8,
			self.day() as u8,
...
impl Encode<Mysql> for time::OffsetDateTime {
	fn encode(&self, out: &mut MysqlParamWriter<'_>) -> Result<(), MysqlError> {
		...
		let year =
			u16::try_from(utc.year()).map_err(|_| MysqlError::Conversion("time year out of range"))?;
		out.push(Value::Date(
			year,
			u8::from(utc.month()),
			utc.day(),
...
impl Decode<Mysql> for time::OffsetDateTime {
	fn decode(row: &mut <Mysql as Backend>::RowReader<'_>) -> Result<Self, MysqlError> {
		let (year, month, day, hour, minute, second, micros) = take_date(row)?;
		let month = time::Month::try_from(month).map_err(|_| MysqlError::Conversion("time month"))?;
...
impl Encode<Mysql> for std::time::SystemTime {
	fn encode(&self, out: &mut MysqlParamWriter<'_>) -> Result<(), MysqlError> {
		...
		let year =
			u16::try_from(year).map_err(|_| MysqlError::Conversion("SystemTime year out of range"))?;
...
/// Renders SQL into a freshly allocated string, returning a render reject (a query shape MySQL cannot
/// render, such as a scoped recursive CTE arm) as an [`io::Error`](std::io::Error) instead of
...
mod tests {
	...
	fn chrono_encodes_as_utc_value_date() {
		let ts = chrono::DateTime::<chrono::Utc>::from_timestamp(1_700_000_000, 123_456_000).unwrap();
		assert_eq!(
	...
	fn timestamps_outside_mysql_range_are_rejected() {
		#[cfg(feature = "chrono")]
		{
			let post_2038 = chrono::DateTime::<chrono::Utc>::from_timestamp(2_147_483_648, 0).unwrap();
