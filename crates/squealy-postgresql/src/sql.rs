...
use squealy::{
    ArithmeticOp, AssignmentValueRef, BindSink, BindValue, CheckModel, Column, ColumnDefault,
    ColumnModel, ColumnRef, ColumnType, CompareOp, DatabaseModel, DefaultValue, Expr, ExprAst,
    ExprKind, ExprVisitor, ForeignKeyModel, Index, IndexModel, InsertAssignments, InsertableTable,
    Order, OrderDirection, Predicate, PredicateAst, PredicateAstVisitor, PredicateKind,
    PredicateNodes, PredicateVisitor, Projectable, ProjectionShape, ProjectionVisitor,
    QueryBuilder, SchemaTable, SelectAst, SelectSink, Selected, SourceAlias, SqlType, Table,
    TableModel, TableProjection, UpdateAssignments, UpdateableTable,
};
...
fn write_model_column(column: &ColumnModel, writer: &mut impl Write) -> io::Result<()> {
    write_quoted_ident(&column.name, writer)?;
    write!(writer, " {}", postgres_sql_type(&column.ty))?;
    if column.auto_increment {
        writer.write_all(b" GENERATED BY DEFAULT AS IDENTITY")?;
    }
    if !column.nullable {
...
/// Maps the owned, neutral [`SqlType`] to a PostgreSQL DDL type. Mirrors
/// [`postgres_column_type`] for the compile-time [`ColumnType`].
fn postgres_sql_type(ty: &SqlType) -> &str {
    match ty {
        ...
        SqlType::U8 => "smallint",
        SqlType::U16 => "integer",
        SqlType::U32 | SqlType::U64 | SqlType::U128 | SqlType::Usize => "bigint",
        SqlType::F32 => "real",
...
fn postgres_column_type(column_type: ColumnType) -> &'static str {
    match column_type {
        ...
        ColumnType::I64 | ColumnType::I128 | ColumnType::Isize => "bigint",
        ColumnType::U8 => "smallint",
        ColumnType::U16 => "integer",
        ColumnType::U32 | ColumnType::U64 | ColumnType::U128 | ColumnType::Usize => "bigint",
        ColumnType::F32 => "real",
...
mod tests {
    ...
    #[test]
    fn postgres_column_types_map_to_postgres_ddl_types() {
        let cases = [
            (ColumnType::Bool, "boolean"),
            ...
            (ColumnType::I128, "bigint"),
            (ColumnType::Isize, "bigint"),
            (ColumnType::U8, "smallint"),
            (ColumnType::U16, "integer"),
            (ColumnType::U32, "bigint"),
            (ColumnType::U64, "bigint"),
            (ColumnType::U128, "bigint"),
        ...
        for (column_type, db_type) in cases {
            assert_eq!(postgres_column_type(column_type), db_type);
        }
    }
}
