...
use squealy::{
    ...
    ColumnModel, ColumnRef, ColumnType, CompareOp, DatabaseModel, Expr, ExprAst, ExprKind,
    ExprVisitor, ForeignKeyModel, Index, IndexModel, InsertAssignments, InsertableTable, Order,
    ...
    SelectAst, SelectSink, Selected, SourceAlias, Table, TableModel, TableProjection,
...
fn write_model_column(column: &ColumnModel, writer: &mut impl Write) -> io::Result<()> {
    write_quoted_ident(&column.name, writer)?;
    write!(writer, " {}", postgres_column_type(column.ty))?;
    if column.auto_increment {
        writer.write_all(b" GENERATED BY DEFAULT AS IDENTITY")?;
    }
    if !column.nullable {
    ...
    if let Some(default) = &column.default {
        writer.write_all(b" DEFAULT ")?;
        write_default(default.clone(), writer)?;
    }
    Ok(())
}
