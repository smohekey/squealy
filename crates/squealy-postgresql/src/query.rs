...
use squealy::{
	...
	ExecutableUpdateFromQuery, ExecutableUpdateQuery, HAppend, HList, HNil, InsertQuery, InsertRows,
...
use tokio_postgres::{
	types::{to_sql_checked, FromSql, FromSqlOwned, IsNull, ToSql, Type},
	GenericClient,
};
...
pub trait PostgresExecutor: Connection<Backend = Postgres> {
	...
	fn prepare_sql<'query>(
		&'query self,
		sql: String,
	) -> Pin<Box<dyn Future<Output = Result<tokio_postgres::Statement, PostgresError>> + Send + 'query>>;

	fn query_raw<'query>(
		&'query self,
		sql: String,
		params: Vec<PostgresParam>,
	) -> Pin<Box<dyn Future<Output = Result<tokio_postgres::RowStream, PostgresError>> + Send + 'query>>;
...
{
	fn query_with_params(connection: &'query Conn, sql: String, params: Vec<PostgresParam>) -> Self {
		Self {
	...
	T: serde::Serialize,
{
	fn encode(&self, out: &mut PostgresParamWriter<'_>) -> Result<(), PostgresError> {
		let value =
			serde_json::to_value(&self.0).map_err(|_| PostgresError::Conversion("serialize jsonb"))?;
	...
	T: serde::de::DeserializeOwned,
{
	fn decode(row: &mut <Postgres as Backend>::RowReader<'_>) -> Result<Self, PostgresError> {
		...
		let inner =
			serde_json::from_value(value).map_err(|_| PostgresError::Conversion("deserialize jsonb"))?;
		Ok(Json(inner))
	}
}
...
impl PostgresNumericInteger {
	...
	fn write_digits<'digits>(&self, digits: &'digits mut [u16; Self::MAX_DIGITS]) -> &'digits [u16] {
