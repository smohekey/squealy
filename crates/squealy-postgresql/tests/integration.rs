...
async fn create_table<S>(client: &tokio_postgres::Client, _backend: &Postgres)
where
	S: SchemaTable,
	S::WithColumn<'static, ColumnName>: Table + Sync,
{
	let table = S::column_names();
	let ddl = match table.name() {
		"integration_users" => {
			"CREATE TABLE integration_users (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name text NOT NULL)"
		}
		"transaction_users" => {
			"CREATE TABLE transaction_users (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name text NOT NULL)"
		}
		"integration_defaulteds" => {
			"CREATE TABLE integration_defaulteds (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name text NOT NULL DEFAULT 'anonymous', score integer NOT NULL DEFAULT 42, active boolean NOT NULL DEFAULT TRUE)"
		}
		"integration_nullables" => {
			"CREATE TABLE integration_nullables (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, note text)"
		}
		"join_users" => {
			"CREATE TABLE join_users (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name text NOT NULL)"
		}
		"join_posts" => {
			"CREATE TABLE join_posts (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, user_id integer NOT NULL REFERENCES join_users(id), title text NOT NULL)"
		}
		"integration_typess" => {
			"CREATE TABLE integration_typess (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, small smallint NOT NULL, medium integer NOT NULL, large bigint NOT NULL, signed_wide numeric NOT NULL, unsigned_large numeric NOT NULL, unsigned_wide numeric NOT NULL, single real NOT NULL, double double precision NOT NULL, flag boolean NOT NULL, label varchar(64) NOT NULL)"
		}
		"integration_newtypes" => {
			"CREATE TABLE integration_newtypes (id integer PRIMARY KEY, parent_id integer, name text NOT NULL)"
		}
		"integration_uuid_records" => {
			"CREATE TABLE integration_uuid_records (id uuid PRIMARY KEY, name text NOT NULL)"
		}
		"integration_bytes_crates" => {
			"CREATE TABLE integration_bytes_crates (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, payload bytea NOT NULL, maybe bytea)"
		}
		"integration_bytess" => {
			"CREATE TABLE integration_bytess (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, data bytea NOT NULL, maybe bytea)"
		}
		"integration_fixed_bytess" => {
			"CREATE TABLE integration_fixed_bytess (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, key bytea NOT NULL CHECK (octet_length(key) = 4), nonce bytea CHECK (octet_length(nonce) = 2))"
		}
		"integration_timeds" => {
			"CREATE TABLE integration_timeds (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, created timestamp with time zone NOT NULL)"
		}
		"integration_upserts" => {
			"CREATE TABLE integration_upserts (id integer PRIMARY KEY, name text NOT NULL)"
		}
		other => panic!("missing handwritten fixture DDL for {other}"),
	};
