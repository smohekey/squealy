...
fn test_window_ranking_functions_render() {
	let row_num = TestConnection
		.from::<Post>()
		.select(|(post,)| row_number().over(|w| w.partition_by(post.user_id).order_by(post.id.asc())));
	assert_eq!(
		row_num.to_sql(),
...
fn named_window_reference_sets_contains_named_window_flag() {
	...
	let _q = TestConnection
		...
		.select_over(|(post,), w| {
			...
			*inline.borrow_mut() = Some(post.user_id.sum().over(|w2| w2.partition_by(post.user_id)));
			...
			*plain_case.borrow_mut() = Some(case().when(post.id.greater_than(0), 1i64).otherwise(0i64));
